...
def scrape_html_race(race_id_list: list, skip: bool):
    """
...
def scrape_html_horse(horse_id_list: list, skip: bool):
    """
...
def scrape_html_ped(horse_id_list: list, skip: bool):
    """
...
def scrape_html_horse_with_master(horse_id_list: list, skip: bool):
    """
    netkeiba.comのhorseページのhtmlをスクレイピングしてdata/html/horseに保存する関数。
    skip=Trueにすると、すでにhtmlが存在する場合はスキップされ、Falseにすると上書きされる。
    返り値：新しくスクレイピングしたhtmlのファイルパス
